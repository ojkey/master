...
tm_kod_webcam_WebCam = function () {
    ...
    connector.freeze = function () {
        webCam.freeze();
    };
    connector.unfreeze = function () {
        webCam.unfreeze();
    };
    connector.reset = function () {
        webCam.reset();
    };
    connector.snap = function () {
        webCam.snap(function (data_uri) {
            var raw_image_data = data_uri.replace(/^data\:image\/\w+\;base64\,/, '');
            connector.onSave(raw_image_data);
        });
    };
};